...
exports.login = async (req, res) => {
    try{
        ...
        if(!user){
            return res.status(401).json({
                success:false,
                message:"User is not registered with us.Please check the crenditals",
            });
        }
        ...
        if(await bcrypt.compare(password, user.password)) {
            ...
            return res.status(401).json({
                success:false,
                message:"Password does not matched.Please check your password",
            });
        }
        ...
        return res.status(500).json({
            success:false,
            message:"User cannot logged in.Please try again later",
        }) 
    }
} 
